...
export let command: SlashCommand = {
    settings: {
        ...
        command: new SlashCommandBuilder()
            ...
            .setDescription("Узнать топ 10 по балансу."),
        run: async (_: Client<boolean>, interaction: ChatInputCommandInteraction, database: Db) => {
            ...
            const users = await usersCollection.find({ balance: { $gt: -1 } }).sort({ balance: -1 }).limit(10).toArray() as any as Array<Users>;
            const parsedUsers: Array<string> = [];
            const emojis = ["👑", "🔥", "😎", "🌟", "💎", "⚡", "🤟", "💥", "✨", "🍀"];

            for (let i = 0; i < 10; i++)
                parsedUsers.push(`${emojis[i]} ${!users[i] ? "N/A" : `<@${users[i].id}>`} ${emojis[i]}\r\n\r\nБаланс: __\`${users[i]?.balance ?? "N/A"} 💵\`__`);
